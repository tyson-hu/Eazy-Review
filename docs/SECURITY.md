...
- Connected rating writes must use **insert vs score-only update** (or a controlled security-definer helper). Do not use PostgREST `.upsert()` payloads that require UPDATE on `product_id` / `user_id`.
