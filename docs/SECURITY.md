...
- Connected rating writes must use a controlled security-definer helper **or** insert vs score-only update with unique-conflict retry (`23505` → score/private-note-only update). Do not use PostgREST `.upsert()` payloads that require UPDATE on `product_id` / `user_id`.
