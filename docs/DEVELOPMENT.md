...
Use Node.js 24 (matching CI) and npm `>=11.16.0 <12` (CI pins `11.17.0`).
Review [install and executable trust rules](SECURITY.md#install-and-setup-scripts)
before installing dependencies or running repository checks. From the repository root:
...
Install Docker Desktop and the Supabase CLI, then follow [Local Supabase](#local-supabase)
to start the local backend and populate the disposable development catalog.
Set only the local public URL and publishable/legacy anon key in `.env`.
Keep configuration local; do not paste keys or environment files into issues.

Start the web preview with `npm run web`. For native development, use the
[physical iPhone setup](#physical-iphone-development-and-offline-testing)
below, or `npm run ios` / `npm run android` with the corresponding local
Xcode / Android toolchain. After the first native build, use Metro for daily
JavaScript edits. All commands below run from the repository root.

## Quality Checks

Otty is optional for visible command panes and scoped terminal readback across
coding tools. See the [Otty workflow and setup limits](MCP_WORKFLOW.md#optional-otty-terminal-workflow).

Validation commands and when to use each live in `AGENT_WORKFLOW.md` (Validation). For CI or local web-bundle verification: `CI=1 npx expo export --platform web`.
...
The reset replaces data in the disposable local development database; use it
only when that local data can be discarded.
...
Keep Supabase running while using Expo. At the end of your development session,
...
[`.env.example`](../.env.example)). Copy to `.env` and set:
...
[`.env.example`](../.env.example) values are intentionally invalid placeholders so misconfigured
boots fail with a clear development error. Never put a service-role key in the
mobile bundle (`SECURITY.md`). Remote staging actions require a separate
...
`evidence/task-11-12-database-acceptance/RESULT.md`.
...
Controlled no-plugin reproduction evidence from the Task 15 implementation is in
`evidence/task-15-public-catalog/RESULT.md` (Xcode 27 compatibility
...
`decisions/2026-08-07-temporary-ios-device-build-cng-plugin.md`.
...
`evidence/task-18-password-recovery/RESULT.md`.
...
4. Create an untracked `.env.local` (gitignored via `.env.*`; do not commit
   ...
   [`.env.example`](../.env.example) placeholders, and never a service-role key
   (`SECURITY.md`).
...
acceptance from Task 15 (2026-08-07) is recorded in
`evidence/task-15-public-catalog/RESULT.md`.

## Documentation Discipline

Doc-update rules live in `DOCUMENTATION_POLICY.md`; apply them before commit/PR handoff.

Browser-based development, research and verification use ego-browser. See
[tool policy](MCP_WORKFLOW.md) and the
[web preview procedure](WEB_MOBILE_PREVIEW_SOP.md). Native and physical
