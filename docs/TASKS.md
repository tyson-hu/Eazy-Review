...
- PostgreSQL constraints: scores 1–10; `private_note` max 500 chars; one rating per user per product; required timestamps; valid FKs; `product_offers.price` null or a finite `>= 0` value (negative and `'NaN'::numeric` rejected; null remains unavailable).
...
- Required constraints (including `product_offers.price` null or finite `>= 0`, rejecting negatives and `'NaN'::numeric`), `is_published`, RLS-on-create (deny-by-default), aggregate `REVOKE`s, per-product serialized aggregate refresh, and `auth.users` → `profiles` ensure trigger (see `docs/DATA_MODEL.md`).
