...
- `npm run decisions:check` — validates decision filenames, metadata, statuses, IDs, acyclic supersession links (no self-references or replacement cycles), exact required section headings outside fenced code, the legacy archive, and generated-index freshness without writing. Run after decision-record or index-tooling edits; CI and `npm run check` also run it.
