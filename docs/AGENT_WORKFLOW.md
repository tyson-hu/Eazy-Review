...
- `npm run decisions:check` — validates decision filenames (rejects misnamed candidates; lowercase `.md` only), metadata, statuses, IDs, positive safe-integer task numbers, acyclic supersession links that terminate at an `accepted` record (multi-generation chains allowed; intermediate replacements may stay `superseded` with historical `supersedes`), exact unfenced level-one title (no `[` / `]` / `\` delimiters) and required section headings outside backtick/tilde fenced code (each required section body bounded by the next unfenced `##` heading), legacy-archive SHA-256 integrity over LF-normalized UTF-8, and generated-index freshness after the same LF normalization (so CRLF working trees do not false-fail). Run after decision-record or index-tooling edits; CI and `npm run check` also run it.
