...
- `npm run check:skill-wrappers` — `skills/manifest.json` is the authoritative skill inventory; canonical `skills/`, `.agents/skills`, `.claude/skills`, the `AGENTS.md` Skill Index, and `docs/LOOP_ENGINEERING.md` skill paths must match it. Discovery stubs have YAML front matter parsed as YAML with non-empty **string** `name`/`description` (rejects null, booleans, and every YAML numeric spelling including hex/octal/binary), include an exact Markdown code span for the canonical `skills/<name>/SKILL.md` path (not a prefixed/suffixed substring), and stay byte-identical across wrapper roots. Fast; run after any skill or skill-wrapper edit, and always as part of `npm run check`.
