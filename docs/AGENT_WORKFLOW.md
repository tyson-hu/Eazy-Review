...
- `npm run decisions:check` — validates decision filenames (rejects misnamed candidates; lowercase `.md` only), metadata, statuses, IDs, acyclic supersession links that require an `accepted` replacement (no self-references or replacement cycles), exact unfenced level-one title and required section headings outside backtick/tilde fenced code, legacy-archive SHA-256 integrity, and generated-index freshness without writing. Run after decision-record or index-tooling edits; CI and `npm run check` also run it.
