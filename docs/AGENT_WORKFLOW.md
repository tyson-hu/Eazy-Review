...
- `npm run decisions:check` — validates decision filenames (rejects misnamed candidates; lowercase `.md` only), metadata, statuses, IDs, acyclic supersession links that require an `accepted` replacement (no self-references or replacement cycles), exact unfenced level-one title (no `[` / `]` / `\` delimiters) and required section headings outside backtick/tilde fenced code, legacy-archive SHA-256 integrity, and generated-index freshness without writing. Run after decision-record or index-tooling edits; CI and `npm run check` also run it.
- `npm run check:skill-wrappers` — discovery stubs under `.agents/skills` and `.claude/skills` have YAML front matter with non-empty **string** `name`/`description` (rejects empty, comment-only, YAML null `null`/`~`, and non-string scalars such as booleans/numbers and special floats `.inf`/`.nan`), include an exact Markdown code span for the canonical `skills/<name>/SKILL.md` path (not a prefixed/suffixed substring), and stay synchronized. Fast; run after any skill or skill-wrapper edit, and always as part of `npm run check`.
