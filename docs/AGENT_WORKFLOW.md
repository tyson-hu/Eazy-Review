...
- `npm run decisions:check` — validates decision filenames (rejects misnamed candidates; lowercase `.md` only), requires `docs/decisions/README.md` to exist (recording guide / template; allowed non-ADR support file, not optional), rejects unexpected subdirectories under `docs/decisions/` (only `archive/` is allowed; ADR files stay flat), metadata, statuses, IDs, positive safe-integer task and `pr` numbers, acyclic supersession links that terminate at an `accepted` record (multi-generation chains allowed; intermediate replacements may stay `superseded` with historical `supersedes`), exact unfenced level-one title (no `[` / `]` / `\` delimiters) and required section headings outside backtick/tilde fenced code and HTML comments (HTML comments are stripped before fence detection so commented fence markers cannot hide following headings; each required section body bounded by the next unfenced `##` heading and requiring at least one substantive visible unfenced line — empty fences, HTML comments, empty Markdown block markers such as `-` / `>` / `1.`, thematic breaks such as `---` / `***` / `___` / `* * *`, raw HTML with no remaining text such as `<div></div>` / `<br>`, and link-reference definitions alone do not count), legacy-archive SHA-256 integrity over LF-normalized UTF-8, and generated-index freshness after the same LF normalization (so CRLF working trees do not false-fail). Run after decision-record or index-tooling edits; CI and `npm run check` also run it.
- `npm run check:skill-wrappers` — `skills/manifest.json` is the authoritative skill inventory; canonical `skills/`, `.agents/skills`, `.claude/skills`, the `AGENTS.md` Skill Index **inventory list line** (not later prose), and the `docs/LOOP_ENGINEERING.md` **Loop Index** Skill column must match it (Trigger cells must contain visible instruction text — HTML comments alone do not count; unmatched `<!--` / `-->` delimiters are rejected — and must not carry skill paths; prose outside the table does not count). Each Loop Index data row must have exactly one `skills/<name>` in the Skill cell. Each canonical `skills/<name>/SKILL.md` must be a nonempty regular file with a matching top-level heading, `Goal:`, and substantive visible bodies under `## When to use` and `## Routine` (empty headings alone do not count). Discovery stubs have YAML front matter parsed as YAML with non-empty **string** `name`/`description` (rejects null, booleans, and every YAML numeric spelling including hex/octal/binary), must target **exactly** the canonical `skills/<name>/SKILL.md` path as a code span (no other `skills/*/SKILL.md` targets), and stay byte-identical across wrapper roots. Fast; run after any skill or skill-wrapper edit, and always as part of `npm run check`.
