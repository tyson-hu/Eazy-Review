...
- `npm run check:skill-wrappers` — discovery stubs under `.agents/skills` and `.claude/skills` have YAML front matter with non-empty **string** `name`/`description` (rejects empty, comment-only, YAML null `null`/`~`, and non-string scalars such as booleans/numbers and special floats `.inf`/`.nan`), point at existing canonical `skills/*/SKILL.md` paths, and stay synchronized. Fast; run after any skill or skill-wrapper edit, and always as part of `npm run check`.
