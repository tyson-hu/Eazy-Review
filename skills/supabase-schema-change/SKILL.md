...
3. Apply the correct packet defaults:
   ...
   - **Task 12 (policies + grants):** add published-product policies and owner-only `user_ratings` policies from `docs/DATA_MODEL.md`, **then** least-privilege `GRANT`s for `anon` / `authenticated` (including `UPDATE` on `profiles` for own-profile edits). Never grant client roles before the authorizing policies exist.
