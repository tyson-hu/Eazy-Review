...
- Any change requiring SQL, a migration, or an RLS policy: the tables `profiles`, `products`, `product_images`, `eazy_assessments`, `user_ratings`, `rating_aggregates`, `product_offers`, the `refresh_rating_aggregates` function, or the `user_ratings_refresh_aggregates_trigger` / `user_ratings_identity_immutable` triggers.
...
- Screen work that merely consumes existing data without schema/RLS changes: use `skills/ui-screen-builder` or `skills/feature-slice-builder` (connected-read / auth-connected / connected-write modes when the task packet names them).

## Inputs expected

- The schema or policy change needed and why (which task or bug requires it).
- Whether it affects existing data.
- Which task packet owns the change (`docs/TASKS.md` Tasks 11–18). Do not put Task 12 policies or client `GRANT`s into a Task 11 migration. Task 12 must add a new forward-only migration after Task 11's deny-by-default migration is applied and verified.
...
3. Apply the correct packet defaults:
   - **Task 11 (schema create):** enable RLS on every new exposed table in the same migration (deny-by-default). Do **not** add public/catalog policies and do **not** `GRANT` to `anon` or `authenticated`. Optional `service_role` tooling grants only. Include `product_offers` in the first schema slice with `size` / `price` null-or-finite-`>= 0` (reject negatives and `'NaN'::numeric`), `size_region text not null default 'US' check (size_region in ('US'))`, and `currency text not null default 'USD' check (currency in ('USD'))`. Ship versioned `eazy_assessments` with required `is_current` and the one-current-per-product partial unique index (overwrite-only is rejected). Revoke `EXECUTE` on aggregate helpers from `PUBLIC` / `anon` / `authenticated`. Serialize per-product aggregate refresh (advisory/xact lock before reading `user_ratings`). Ensure a zero-count `rating_aggregates` row on `products` INSERT. Add `handle_new_user` on `auth.users` AFTER INSERT to create `public.profiles (id)` only (revoke client EXECUTE). Include `user_ratings_identity_immutable` (protects `product_id` and `user_id`). Environment setup files (`supabase/config.toml`, env example/validation, package scripts/lockfile for CLI/secret-scan) are in scope when required by the Task 11 checklist — not UI screens.
   - **Task 12 (policies + grants):** create a **new** migration (never edit the applied Task 11 migration). Add published-product catalog policies and owner-only `user_ratings` policies from `docs/DATA_MODEL.md` (INSERT/UPDATE also require `is_published`), **then** least-privilege `GRANT`s for `anon` / `authenticated` — including **column-level** `UPDATE` on `profiles` (`display_name`, `username`, `avatar_url`) and column-level `user_ratings` grants: INSERT permits `product_id`, `user_id`, score columns and `private_note`; UPDATE permits only score columns and `private_note`. Never grant client roles before the authorizing policies exist. Keep `updated_at` trigger-maintained (no client UPDATE grant on timestamps / `id`). Identity columns remain immutable after insertion and must never appear in the UPDATE set. Task 16 rating writes must not use PostgREST upsert of identity columns; split insert/update paths must retry score-only update on unique violation `23505`. A preferred `SECURITY DEFINER` save helper derives `user_id` from `auth.uid()` rather than trusting client input.
4. Keep Community Score recalculation server-side: any change touching `user_ratings` aggregates goes through `refresh_rating_aggregates` and its trigger, never client code. Keep `user_ratings.product_id` and `user_ratings.user_id` immutable.
