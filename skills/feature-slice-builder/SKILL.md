...
- The matching flow and route requirements in `docs/USER_FLOWS.md` (for example Browse is `app/(tabs)/browse.tsx`; Product Detail is `app/product/[id]/index.tsx`; Rating Form is `app/product/[id]/rate.tsx`; auth/recovery and Rated Products routes as documented).
...
4. Choose the data mode from the task packet:
   ...
   - **Auth-connected (Task 15):** wire email (or approved) auth provider operations — sign-up, sign-in, sign-out, session persistence, password recovery routes, auth-state UI on Account / Rate gate, and local session/cache cleanup on auth change. Implement the **Delete Account product flow** (confirmation UI + call to a protected server-side deletion path that never embeds a service-role key in the Expo bundle). Do **not** delete accounts yourself via MCP, SQL consoles, or admin APIs while building the feature — those actions are FORBIDDEN on every environment. If the Task 14 transitional session rating map still exists, namespace it by signed-in user and clear or re-key on auth change.
...
- For auth-connected mode: confirm sign-up creates the profile row via the existing trigger; Rate CTA gates logged-out users; recovery and delete-account paths match the task acceptance without embedding service-role credentials; no MCP/account-admin deletion was performed as a substitute for the product flow.
