...
Goal: change frontend product/rating data shapes, types, or scoped fixtures — keeping types, fixtures, and contracts in sync with no SQL involved.

## When to use

- Changes to `Product`, `ProductCardData`, `ProductDetailPublicData`,
  `RatingBreakdown`, or `ProductRatingSummary` in `src/types/product.ts`.
- Changes to purpose-built frontend product or rating test fixtures.
...
- The shape change needed and which screens, functions, or scoped fixtures consume it.

## Read first

- `docs/API_CONTRACTS.md`: Frontend Product Types, Product Card Shape, and Scoped Product Fixtures.
...
2. Keep `ProductCardData` limited to the documented Browse fields. Do not add
   Detail-only image arrays, assessments, rating summaries, or offer lists.
...
4. Update `src/types/product.ts`, affected scoped fixtures, and the type blocks
   in `docs/API_CONTRACTS.md` together in the same change.
...
- Making scoped fixtures richer than the contract (fields the real API will never return).
